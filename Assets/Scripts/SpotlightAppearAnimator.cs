...
public class SpotlightAppearAnimator : MonoBehaviour
{
    ...
    IEnumerator Scale()
    {
        ...
        while (max_scale.x > transform.localScale.x || max_scale.y > transform.localScale.y || max_scale.z > transform.localScale.z)
        { // While object not reached maximum scale by each axis
        ...
        //when growing stops 
        GiftUIManager.Instance.OpenRingAndRingAfterSpotlightGrow();
        if (max_scale.x < transform.localScale.x || max_scale.y < transform.localScale.y || max_scale.z < transform.localScale.z)
        { // Size accuracy correction
